...
"""
Implementation of E2C: http://arxiv.org/abs/1506.07365
...
import os
from plane_data2 import PlaneData, get_params
...
def sampleQ_psi(z,u,Q_phi):
  ...
  with tf.variable_scope("sampleQ_psi"):
    ...
    mu=Amu+Bu+o
    Q_psi=NormalDistribution(mu,Q_phi.sigma,Q_phi.logsigma, v, r)
    x=tf.expand_dims(sampleNormal(0,Q_phi.sigma),-1)
    z_next=mu+tf.squeeze(tf.batch_matmul(A,x),[-1])
...
ckpt_file="/tmp/e2c-plane" # checkpoint prefix
...
if __name__=="__main__":
  ...
  sess.run(init)
  # WRITER
  writer = tf.train.SummaryWriter("/tmp/e2c", sess.graph_def)
  dataset=PlaneData("plane2.npz","env1.png")
  dataset.initialize()

  #saver.restore(sess, "/ltmp/e2c-plane-99000.ckpt")
  train_iters=1e5 # 5K iters
  for i in range(int(train_iters)):
    ...
    if i % 1000 == 0:
      saver.save(sess,ckpt_file+"-%05d"%(i)+".ckpt") # keep all checkpoint files
  sess.close()
